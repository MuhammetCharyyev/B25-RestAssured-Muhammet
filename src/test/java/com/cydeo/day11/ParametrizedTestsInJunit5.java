...
public class ParametrizedTestsInJunit5 {
    ...
    @ParameterizedTest
    //tell to Junit to run test with different parameters like Scenario Outline
    ...
    public void testMultipleNumbers(int number){
        ...
        //it will check each int and test will fail if it is > 5
        //something like loop
    ...
    public void testZipcodes(int zipCode){
        ...
        given()
                ...
                .when()
                .get("/us/{zipcode}")
                //each zipcode will be inserted into our request instead of {zipcode}
                .then().log().all()
                .statusCode(200);
