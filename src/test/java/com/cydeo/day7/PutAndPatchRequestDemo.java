...
public class PutAndPatchRequestDemo extends SpartanTestBase {
    ...
    @Test
    public void test1(){
        //just like post request we have different options to send json body,
        // we will go with map
        ...
        given().contentType(ContentType.JSON)
                ...
                //put everything above stated as new record
                .then().statusCode(204);
    ...
    public void test2(){
        ...
        // we add '.patch' instead of '.put' and it allows us to modify our json body

        given().contentType(ContentType.JSON)
                .body(putRequestMap).log().body()
                .and().pathParam("id",85)//we modify data for id=85
                .when().patch("/api/spartans/{id}")//put '.patch' here
                .then().statusCode(204);
    ...
    public void test3(){
        ...
        int idToDelete = 333;//exact id number we wanna delete

        given().pathParam("id",idToDelete)
                .when().delete("/api/spartans/{id}")
                //we indicate exact id number which we want to delete and '.delete'
                .then().statusCode(204);//verify code
